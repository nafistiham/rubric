...
#[test]
fn detects_alias_method_keyword() {
...
#[test]
fn no_violation_for_alias_keyword() {
...
fn does_not_flag_alias_in_comment() {
    let src = "# alias_method :foo, :bar\nclass Foo; end\n";
    let ctx = LintContext::new(Path::new("test.rb"), src);
    let diags = Alias.check_source(&ctx);
    assert!(diags.is_empty(), "alias_method in comment should not be flagged, got: {:?}", diags);
}

#[test]
fn does_not_flag_alias_keyword() {
    let src = "alias new_name old_name\n";
    let ctx = LintContext::new(Path::new("test.rb"), src);
    let diags = Alias.check_source(&ctx);
    assert!(diags.is_empty(), "alias keyword should not be flagged, got: {:?}", diags);
}

#[test]
fn flags_alias_method_with_message() {
    let src = "alias_method :new_name, :old_name\n";
    let ctx = LintContext::new(Path::new("test.rb"), src);
    let diags = Alias.check_source(&ctx);
    assert!(!diags.is_empty(), "alias_method should be flagged");
