...
// ── Symmetric (first param on def line): closing `)` on last param line is fine ────
// `def foo(bar,\n  baz)` — `(` has content on def line, `)` also with content.
// RuboCop `symmetrical` style: both openings/closings are on content lines → OK.
#[test]
fn no_false_positive_symmetric_def_first_param_on_def_line() {
    let src = "def foo(bar,\n        baz)\n  bar + baz\nend\n";
    let ctx = LintContext::new(Path::new("test.rb"), src);
    let diags = MultilineMethodDefinitionBraceLayout.check_source(&ctx);
    assert!(diags.is_empty(), "symmetric multiline def (first param on def line) should not be flagged: {:?}", diags);
}

// ── True positive: asymmetric — first param on new line, `)` on content line ─────
// `def foo(\n  bar,\n  baz)\n` — `(` alone on def line, `)` with content → asymmetric.
#[test]
fn still_detects_asymmetric_def_closing_paren() {
    let src = "def foo(\n  bar,\n  baz)\n  bar + baz\nend\n";
    let ctx = LintContext::new(Path::new("test.rb"), src);
    let diags = MultilineMethodDefinitionBraceLayout.check_source(&ctx);
    assert!(!diags.is_empty(), "asymmetric multiline def (first param on new line, `)` with content) should be flagged");
}
